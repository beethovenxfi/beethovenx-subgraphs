...
export function deposit(event: Deposit): void {
  ...
  user.rewardDebt = user.rewardDebt.plus(
    event.params.amount.times(pool.accBeetsPerShare).div(ACC_BEETX_PRECISION)
  );
  user.save();
}
...
export function withdraw(event: Withdraw): void {
  ...
  user.rewardDebt = user.rewardDebt.minus(
    event.params.amount.times(pool.accBeetsPerShare).div(ACC_BEETX_PRECISION)
  );
  user.save();
...
export function harvest(event: Harvest): void {
  log.info("[MasterChefV2] Log Harvest {} {} {}", [
    event.params.user.toHex(),
    event.params.pid.toString(),
    event.params.amount.toString(),
  ]);
  ...
  const harvest = new HarvestAction(
    `${user.id}-${masterChef.beets.toHex()}-${event.block.timestamp}`
  );
  harvest.user = user.id;
  harvest.token = masterChef.beets;
  ...
  const rewardDebt = user.amount
    .times(pool.accBeetsPerShare)
    .div(ACC_BEETX_PRECISION);
