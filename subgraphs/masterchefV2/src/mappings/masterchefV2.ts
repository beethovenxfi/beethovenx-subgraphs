...
export function harvest(event: Harvest): void {
    ...
    const masterChef = getMasterChef(event.block)
    const user = getUser(event.params.user, event.params.pid, event.block)
    const harvest = new HarvestAction(`${user.id}-${masterChef.beets.toHex()}-${event.block.timestamp}`)
    ...
    const rewardDebt = user.amount.times(pool.accBeetsPerShare).div(ACC_BEETX_PRECISION)
